...
namespace Monitorian.Test
{
	...
	public class MonitorConfigurationTest
	{
		...
		public void TestMonitorCapability_D1_1()
		{
			...
			var (brightness, contrast, speakerVolume) = TestEnumerateVcpCodes(source);
			Assert.IsTrue(brightness);
			Assert.IsTrue(contrast);
			Assert.IsFalse(speakerVolume);

			var (temperature, values) = TestGetVcpCodeValues(source);
			Assert.IsTrue(temperature);
			Assert.IsTrue(AreIncluded<byte>(values, 4, 5, 6, 8, 9));
		}
		...
		public void TestMonitorCapability_D1_2()
		{
			...
			var (temperature, values) = TestGetVcpCodeValues(source);
			Assert.IsTrue(temperature);
			Assert.IsTrue(AreIncluded<byte>(values, 5, 8));
		}
		...
		public void TestMonitorCapability_D1_3()
		{
			...
			var (temperature, values) = TestGetVcpCodeValues(source);
			Assert.IsTrue(temperature);
			Assert.IsTrue(AreIncluded<byte>(values, 4, 5, 8));
		}
		...
		public void TestMonitorCapability_N1_1()
		{
			...
			var (temperature, values) = TestGetVcpCodeValues(source);
			Assert.IsTrue(temperature);
			Assert.IsTrue(AreIncluded<byte>(values, 2, 6, 8));
		}
		...
		public void TestMonitorCapability_L1_1()
		{
			...
			var (temperature, values) = TestGetVcpCodeValues(source);
			Assert.IsTrue(temperature);
			Assert.IsTrue(AreIncluded<byte>(values, 5, 6, 7, 8));
		}
		...
		public void TestMonitorCapability_L2_1()
		{
			...
			var (temperature, values) = TestGetVcpCodeValues(source);
			Assert.IsTrue(temperature);
			Assert.IsTrue(AreIncluded<byte>(values, 2, 5, 6, 8));
		}
		...
		public void TestMonitorCapability_L2_2()
		{
			...
			var (temperature, values) = TestGetVcpCodeValues(source);
			Assert.IsTrue(temperature);
			Assert.IsTrue(AreIncluded<byte>(values, 5, 6, 8));
		}
		...
		public void TestMonitorCapability_A2_1()
		{
			...
			var (temperature, values) = TestGetVcpCodeValues(source);
			Assert.IsTrue(temperature);
			Assert.IsTrue(AreIncluded<byte>(values, 3, 5, 9));
		}
		...
		private static (bool brightness, bool contrast, bool speakerVolume) TestEnumerateVcpCodes(string source)
		{
		...
		private static (bool temperature, byte[] values) TestGetVcpCodeValues(string source)
		{
			var @class = new PrivateType(typeof(MonitorConfiguration));
			var vcpCodes = @class.InvokeStatic("GetVcpCodeValues", source) as Dictionary<byte, byte[]>;

			return vcpCodes.TryGetValue((byte)VcpCode.Temperature, out byte[] values)
				? (true, values)
				: (false, Array.Empty<byte>());
		}

		private static bool AreIncluded<T>(T[] source, params T[] elements)
		{
			return source.Intersect(elements).Count() == elements.Length;
		}
	}
}